...
for suggestion in suggestions:
    ...
    try:
        ...
        # Skip this suggestion if the result is out of range
        continue
...
if st == 'Y':
    ...
    for term in terms:
        ...
        # Initialize competition, volume, and score as None in case the data is not available
        comp = None
        score = None
        vol = None

        # Check if the data is available before assigning it to the respective variables
        for i in num:
            if i.startswith('"competition":'):
                comp = int(float(i.split(":")[1]))
            elif i.startswith('"overall":'):
                score = int(float(i.split(":")[1]))
            elif i.startswith('"volume":'):
                vol = int(float(i.split(":")[1]))
